...
public class appiumWebConnector<V> {
    ...
    public appiumWebConnector(){
        try {
            prop.load( new FileInputStream("./src/test/config/mobileapp.properties") );
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
...
//                androidCapabilities.setCapability("udid", prop.getProperty("UDID"));
                ...
                androidCapabilities.setCapability("app", System.getProperty("user.dir")+"/src/test/lib/selendroid-test-app.apk");
                ...
                androidCapabilities.setCapability("appActivity", prop.getProperty("HomeScreenActivity"));
                androidCapabilities.setCapability("appActivity", prop.getProperty("WebViewActivity"));
                androidCapabilities.setCapability("appActivity", prop.getProperty("MultipleWebViewsActivity"));
                androidCapabilities.setCapability("appActivity", prop.getProperty("RegisterUserActivity"));
                androidCapabilities.setCapability("appActivity", prop.getProperty("VerifyUserActivity"));
                androidCapabilities.setCapability("appActivity", prop.getProperty("ExtremLargeActivity"));
                androidCapabilities.setCapability("appActivity", prop.getProperty("SearchUsersActivity"));
...
//                js.executeScript("target.setDeviceOrientation(UIA_DEVICE_ORIENTATION_LANDSCAPELEFT);");
    ...
    public List<WebElement> FindAllElements(String webElement) throws Exception{
        return driver.findElements(getElementWithLocator(webElement));
    }
    public void selectFromList(String webElement, String text) throws Exception {
        List<WebElement> itemlist=FindAllElements(webElement);
    ...
    public String getTextFromScreen(String webElement) throws Exception {
        return FindAnElement(webElement).getText();
    }
    public boolean verifyThatElementIsDisplayed(String webElement) throws Exception {
        if(FindAnElement(webElement).isDisplayed()){
            return true;
        }return false;
    }
    public boolean verifyThatListOfAllElementsAreDisplayed(List<String> webElementList) throws Exception {
        boolean flag=false;
        for(String webElement : webElementList)
        if(!FindAnElement(webElement).isDisplayed()){
            flag=false;
            break;
        }
